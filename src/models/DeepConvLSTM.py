...
class DeepConvLSTMModel(RainbowModel):
    ...
    def _create_model(self, n_features, n_outputs):
        ...
        #Adding 4 CNN layers.
        model.add(Conv2D(NUM_FILTERS_CNN, kernel_size=(FILTER_SIZE_CNN, 1), activation="relu", kernel_initializer = initializer))
        model.add(Conv2D(NUM_FILTERS_CNN, kernel_size=(FILTER_SIZE_CNN, 1), activation="relu", kernel_initializer = initializer))
        model.add(Conv2D(NUM_FILTERS_CNN, kernel_size=(FILTER_SIZE_CNN, 1), activation="relu", kernel_initializer = initializer))
        model.add(Conv2D(NUM_FILTERS_CNN, kernel_size=(FILTER_SIZE_CNN, 1), activation="relu", kernel_initializer = initializer))
        model.add(Permute((2,1,3)))
        model.add(Reshape( (int(model.layers[4].output_shape[1]), int(model.layers[4].output_shape[2]) * int(model.layers[4].output_shape[3]))))
